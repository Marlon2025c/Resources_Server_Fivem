        ...
        while insidegarage do
            for k, v in pairs(Config.Customs) do
                ...
                if Config.InteractiveFeature['paintmenu'] and distance < 30 and ShopPermmision(currentprivate, 'paintmenu') and Config.showmarker then
                    DrawMarkerInput(vector3(v.paintmenu.coord.x, v.paintmenu.coord.y, v.paintmenu.coord.z),
                        'Spray Paint Menu', 'renzu_customs:openpaintmenu', false, 'spray_paint')
                end
                    ...
                    for k, v in pairs(v.mod) do
                        ...
                        if Config.JobPermissionAll and ShopPermmision(currentprivate) or not Config.JobPermissionAll then
                            if distance < 30 and invehicle and Config.showmarker then
                                DrawMarkerInput(vector3(v.coord.x, v.coord.y, v.coord.z), 'Upgrade Menu',
                                    'renzu_customs:openmenu', false, 'mod_' .. k)
                            end
                ...
                if Config.InteractiveFeature['stockroom'] and distance < 30 and ShopPermmision(currentprivate, 'stockroom') and Config.showmarker then
                    DrawMarkerInput(vector3(v.stockroom.coord.x, v.stockroom.coord.y, v.stockroom.coord.z), 'Stock Room',
                        'renzu_customs:openstockroom', false, 'stock_inventory', k)
                end
            ...
            if Config.InteractiveFeature['garage_inventory'] and nearveh ~= 0 and not carrymod and not spraying and ShopPermmision(currentprivate, 'garage_inventory') then
                if stats_show == nil or stats_show ~= nearveh then
                    ...
                    CreateThread(function()
                        while nearveh ~= 0 do
                            ShowFloatingHelpNotification('Press [E] to See Vehicle Parts',
                                GetEntityCoords(nearveh) + vec3(0, 0, 1.0))
            ...
            if Config.InteractiveFeature['garage_inventory'] and inventorydis < 10 and not carrymode and not carrymod and ShopPermmision(currentprivate, 'garage_inventory') and Config.showmarker then
                DrawMarkerInput(vector3(inv.coord.x, inv.coord.y, inv.coord.z), 'Parts Inventory',
                    'renzu_customs:openinventory', false, 'parts_inventory', currentprivate)
            end
            ...
            elseif Config.InteractiveFeature['garage_inventory'] and not carrymode and carrymod and inventorydis < 2 and ShopPermmision(currentprivate, 'garage_inventory') then
                if inventorydis < 2 and ShopPermmision(currentprivate, 'garage_inventory') and Config.showmarker then
                    DrawMarkerInput(vector3(inv.coord.x, inv.coord.y, inv.coord.z), 'Press [E] Store',
                        'renzu_customs:storemod', false, 'store_mod',
                        { currentprivate, Config.VehicleMod[tostore[1]], tostore[2], false, false, true }, true)
                end
...
AddEventHandler('renzu_customs:openinventory', function(current)
    ...
    TriggerServerCallback_("renzu_customs:getinventory", function(inventory)
        ...
        if openmenu then
            TriggerEvent('renzu_contextmenu:insertmulti', multimenu, "Vehicle Parts", false,
                "<i class='fad fa-inventory'></i> Mod Inventory")
...
AddEventHandler('renzu_customs:openstockroom', function(current)
                    ...
                    if max >= 0 then
                        ...
                        for i = 0, max do
                            ...
                            if multimenu[vehicle][v.label].main_fa == nil then
                                multimenu[vehicle][v.label].main_fa =
                                '<img style="height: auto;margin-left: -20px;margin-top: -10px;position: relative;max-width: 35px;float: left;" src="https://cfx-nui-renzu_customs/html/img/' ..
                                k .. '.svg">'
                            end
    ...
    if openmenu then
        TriggerEvent('renzu_contextmenu:insertmulti', localmultimenu, "Vehicle Parts", false,
            "<i class='fad fa-container-storage'></i> Stock Room")
        TriggerEvent('renzu_contextmenu:show')
    end
end)
...
AddEventHandler('renzu_customs:partlist', function(vehicle)
    ...
    if DoesEntityExist(vehicle) and IsEntityAVehicle(vehicle) then
        SetModable(vehicle)
        if openmenu then
            TriggerEvent('renzu_contextmenu:insertmulti', multimenu[vehicle], "Vehicle Parts", false,
                "<i class='fas fa-cog'></i> Part List")
            Wait(100)
            TriggerEvent('renzu_contextmenu:show')
        end
    end
    ...
    if mod ~= nil and GetVehicleMod(vehicle, tonumber(mod.index)) + 1 >= lvl then
        ...
        while carrymode do
            ...
            if distance < 3 and ShopPermmision(currentprivate, 'garage_inventory') and Config.showmarker then
                DrawMarkerInput(vector3(vec.coord.x, vec.coord.y, vec.coord.z), 'Press [E] Store',
                    'renzu_customs:storemod', false, 'store_mod', { currentprivate, mod, lvl, newprop }, true)
            end
        ...
        for i = 0, 49 do
            ...
            if Config.VehicleMod[i] ~= nil then
                ...
                if GetVehicleMod(vehicle, mod.index) + 1 > 0 then
                    ...
                    multimenu[mod.type:upper()].main_fa =
                    '<img style="height: auto;margin-left: -20px;margin-top: -10px;position: relative;max-width: 35px;float: left;" src="https://cfx-nui-renzu_customs/html/img/' ..
                    i .. '.svg">'
                    multimenu[mod.type:upper()][mod.label:upper() .. ' ' .. GetVehicleMod(vehicle, i) + 1] = {
                        ['title'] = mod.label .. ' ' .. GetVehicleMod(vehicle, i) + 1,
                        ['fa'] =
                        '<img style="height: auto;position: absolute;max-width: 30px;left:5%;top:25%;" src="https://cfx-nui-renzu_customs/html/img/' ..
                        i .. '.svg">',
                        ['type'] = 'event', -- event / export
    ...
    if NetworkDoesEntityExistWithNetworkId(net) and tire == 'Default' and vehicle ~= 0 or NetworkDoesEntityExistWithNetworkId(net) and tire == nil and vehicle ~= 0 then
        ...
        if default then
            SetVehicleHandlingFloat(vehicle, "CHandlingData", "fLowSpeedTractionLossMult",
                default.fLowSpeedTractionLossMult + 0.0)                                                                             -- self.start burnout less = traction
            ...
            SetVehicleHandlingFloat(vehicle, "CHandlingData", "fTractionCurveLateral",
                default.fTractionCurveLateral + 0.0)                                                                                 -- curve lateral grip
        end
    end
end)
            ...
            TriggerServerCallback_("renzu_customs:getmoney", function(money)
                ...
                for k, v in pairs(Config.VehicleMod) do
                    if Config.JobPermissionAll and PlayerData ~= nil and PlayerData.job ~= nil and v.job_grade ~= nil and v.job_grade[PlayerData.job.name] ~= nil and PlayerData.job.grade >= v.job_grade[PlayerData.job.name]
                        ...
                        if custom[v.type:upper()] == nil then
                            custom[v.type:upper()] = {}
                            custom[v.type:upper()].index = k
                        end
...
AddEventHandler('renzu_customs:paint', function(color, spray)
    if not spray then
        ...
        AttachEntityToEntity(spraycan, ped, GetPedBoneIndex(ped, 57005), 0.072, 0.041, -0.06, 33.0, 38.0, 0.0, true, true,
            false, true, 1, true)
        local nearveh = nil
        while tospray do
            nearveh = GetClosestVehicle(GetEntityCoords(PlayerPedId()), 2.000, 0, 70)
...
AddEventHandler('renzu_customs:openpaintmenu', function(garage, garage_id)
    ...
    if openmenu then
        TriggerEvent('renzu_contextmenu:insertmulti', localmultimenu, "Vehicle Parts", false,
            "<i class='fas fa-spray-can'></i> Spray Paints")
        TriggerEvent('renzu_contextmenu:show')
    end
end)
