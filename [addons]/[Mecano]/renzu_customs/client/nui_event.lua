...
RegisterNUICallback('repair', function(data, cb)
    ...
    TriggerServerCallback_("renzu_customs:repair", function(ret)
        if ret then
            ...
            TriggerEvent('renzu_notify:Notify', 'error', 'Customs',
                'Not enough money cabron $' .. Config.RepairCost .. ' Required')
            cb(false)
        end
    end, currentprivate)
end)
...
RegisterNUICallback('SetPaint', function(data, cb)
    ...
    SetVehicleModKit(vehicle, 0)
    local primary, secondary = GetVehicleColours(vehicle)
    if data.type == "Couleur primaire" and data.option ~= 'Pearlescent' and data.option ~= 'Chameleon' then
        ClearVehicleCustomPrimaryColour(vehicle)
        SetVehicleColours(vehicle, data.index, secondary)
    elseif data.type == "Couleur primaire" and data.option == 'Pearlescent' and data.option ~= 'Chameleon' then
        local pearlcent, wheel = GetVehicleExtraColours(vehicle)
        SetVehicleExtraColours(vehicle, data.index, wheel)
    elseif data.type == "Couleur secondaire" and data.option ~= 'Pearlescent' and data.option ~= 'Chameleon' then
        ClearVehicleCustomSecondaryColour(vehicle)
        SetVehicleColours(vehicle, primary, data.index)
    elseif data.type == "Couleur primaire" and data.option == 'Chameleon' and data.option ~= 'Pearlescent' then
        ClearVehicleCustomSecondaryColour(vehicle)
        ClearVehicleCustomPrimaryColour(vehicle)
...
RegisterNUICallback('SetCustomColor', function(data)
    ...
    SetVehicleModKit(vehicle, 0)
    local primary, secondary = GetVehicleColours(vehicle)
    if data.type == "Couleur primaire" then
        SetVehicleCustomPrimaryColour(vehicle, r, g, b)
    elseif data.type == "Couleur secondaire" then
...
RegisterNUICallback('GetModData', function(v, cb)
    local vehicle = GetVehiclePedIsIn(PlayerPedId()) ~= 0 and GetVehiclePedIsIn(PlayerPedId()) or
