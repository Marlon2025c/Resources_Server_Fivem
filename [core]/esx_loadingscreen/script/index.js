...
/* --------------------------------------------------------------------------------------- */
/* Pour mute la musique changer les volume de la musique modifier la class de div (on/off) */
/* --------------------------------------------------------------------------------------- */
document.addEventListener("DOMContentLoaded", () => {
    const musicToggle = document.getElementById("music-toggle");
    const backgroundMusic = document.getElementById("background-music");
    const onLabel = document.querySelector(".on");
    const offLabel = document.querySelector(".off");
    ...
    function toggleMusic() {
        if (musicToggle.checked) {
            ...
            // Mettre à jour les couleurs en fonction de l'état
            if (backgroundMusic.muted) {
                onLabel.style.backgroundColor = "red";
                offLabel.style.backgroundColor = "chartreuse";
            } else {
                onLabel.style.backgroundColor = "chartreuse";
                offLabel.style.backgroundColor = "red";
            }
        }
    }
    ...
    document.addEventListener("keydown", (event) => {
        if (event.keyCode === 32) { // 32 correspond à la touche Espace
            toggleMusic();
        }
    });
});

/* bar de chargement loadscreen */
    if (!String.format) {
    ...
    const loadingStages = ["Carte", "Personnage", "Voiture", "Armes"];
